...
/// <para>2、结合<see cref="Interceptor"/>在运行时实现方法拦截，并调用此句柄 </para>
...
public sealed class MethodAspectAttribute : Attribute
{
    /// <summary>
    /// 方法拦截器
    /// <para>1、必传；实现<see cref="Interfaces.IMethodInterceptor"/>的类型依赖注入Key值 </para>
    /// </summary>
    public required string Interceptor { init; get; }
}
