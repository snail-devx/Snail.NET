...
/// <summary>
/// 特性标签：注入配置，在不同对象作用不一样 
/// <para>1、在构造方法上：DI选举构造方法时，优先选择有此特性的第一个构造方法；无此特性则选举参数最长的构造方法</para>
/// <para>2、在属性、字段上：DI构建完实例后，再通过DI自动构建属性、字段值；前提是属性可写</para>
...
public class InjectAttribute : Attribute, IInject
{
    ...
    /// <para>2、默认行为：值为null；在【Constructor】和【Method】使用时忽略此属性 </para>
    /// </summary>
