...
{
    /// <summary>
    /// 接口约束：依赖注入的参数，在DI构建实例时，传递给实现类型的【构造方法】的参数 <br />
    ///     1、仅在【属性、字段、方法参数】的标签上生效，将标记的参数传递到对应实现类的构造方法中 <br />
    ///     2、自定义实现<see cref="GetParameter(in IDIManager)"/>方法；可基于<see cref="IDIManager.Resolve(string?, Type)"/>动态构建值，也可固定值等等
    /// </summary>
    ...
    public interface IParameter<T> : IParameter
    {
        ...
        /// <summary>
        /// 参数类型：和<see cref="IParameter.Name"/>配合时用，选举要传递信息的目标参数 <br />
        /// </summary>
