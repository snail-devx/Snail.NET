...
/// <summary>
/// <see cref="IDbModelProvider{DbModel,IDType}"/>扩展方法
/// </summary>
public static class DbModelProviderExtensions
{
    ...
    public static async Task<DbModel?> Load<DbModel, IdType>(this IDbModelProvider<DbModel, IdType> provider, IdType id)
    ...
    /// <returns>存在返回实体，否则返回null</returns>
    /// <remarks>不支持指定数据分片路由；若需要，请使用<see cref="IDbModelProvider{DbModel,IdType}.AsQueryable(string)"/>方法</remarks>
    public static async Task<DbModel?> Load<DbModel>(this IDbModelProvider<DbModel> provider, string id)
        where DbModel : class
    {
        IList<DbModel> rt = await provider.Load([id]);
        return rt.FirstOrDefault();
