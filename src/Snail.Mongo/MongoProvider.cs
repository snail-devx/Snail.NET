...
/// <summary>
/// <see cref="IDbModelProvider{DbModel, IdType}"/>的MongoDB实现 
...
public class MongoProvider<DbModel, IdType> : DbProvider, IDbModelProvider<DbModel, IdType>
...
/// <summary>
/// <see cref="IDbModelProvider{DbModel}"/>的MongoDB实现 
/// <para>1、强制【瞬时】生命周期，避免不同服务器之间操作实例问题，使用【MongoDB】作为依赖注入key值 </para>
/// <para>2、数据实体的IdType强制为string</para>
/// </summary>
/// <typeparam name="DbModel">数据库实体；需被<see cref="DbTableAttribute"/>特性标记</typeparam>
[Component(From = typeof(IDbModelProvider<>), Key = nameof(DbType.MongoDB), Lifetime = LifetimeType.Transient)]
public class MongoProvider<DbModel> : MongoProvider<DbModel, string>, IDbModelProvider<DbModel> where DbModel : class
{
    #region 构造方法
    /// <summary>
    /// 构造方法
