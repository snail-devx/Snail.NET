...
namespace Snail.Elastic.DataModels
{
    ...
    public class ElasticSortModel : ISerializable
    {
        ...
        /// <summary>
        /// 排序模式
        /// <para>1、在排序字段为array或者多值时生效；无特殊需求null即可；避免传错导致不可用 </para>
        /// <para>2、可选值： </para>
        /// <para>- min（最小值） </para>
        /// <para>- max（最大值） </para>
        /// <para>- sum（最小值，仅数值字段生效） </para>
        /// <para>- avg（平均值，仅数值字段） </para>
        /// <para>- median（中位数，仅数值字段） </para>
        /// </summary>
    ...
    public sealed class ElasticNestedSortFieldModel : ISerializable
    {
        ...
        /// <summary>
        /// 最大用作计算排序的子文档数量。默认不限制；若子文档数量过多，对性能有影响
        /// <para>1、对应ES的“max_children” </para>
        /// </summary>
