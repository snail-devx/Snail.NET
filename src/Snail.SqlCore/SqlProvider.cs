...
/// <summary>
/// <see cref="IDbModelProvider{DbModel,IdType}"/>的【关系型数据库】抽象实现
/// </summary>
/// <typeparam name="DbModel">数据库实体；需被<see cref="DbTableAttribute"/>特性标记</typeparam>
/// <typeparam name="IdType">主键的数据类型，确保和数据实体标记的主键字段类型一致</typeparam>
public abstract class SqlProvider<DbModel, IdType> : DbProvider, IDbModelProvider<DbModel, IdType>, ISqlDbRunner
    where DbModel : class where IdType : notnull
{
    ...
    async Task<bool> IDbModelProvider<DbModel, IdType>.Save(IList<DbModel> models)
    {
        ...
        IdType[] ids = models
            .Select(instance =>
            {
                ...
                return (IdType)DbModelHelper.BuildFieldValue(pkValue, Table.PKField)!;
            })
            .ToArray();
    ...
    /// <remarks>不支持指定数据分片路由；若需要，请使用<see cref="IDbModelProvider{DbMode, IdTypel}.AsDeletable(string)"/>方法</remarks>
    async Task<long> IDbModelProvider<DbModel, IdType>.Delete(params IList<IdType> ids)
    {
        string filterSql = BuildIdFilter(ids, out IDictionary<string, object> param);
    ...
    protected virtual string BuildIdFilter(IList<IdType> ids, out IDictionary<string, object> param)
    {
