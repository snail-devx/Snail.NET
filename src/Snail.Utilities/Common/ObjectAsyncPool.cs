...
public sealed class ObjectAsyncPool<T> : Disposable where T : class, IPoolable
{
    #region 属性变量
    /// <summary>
    ...
    {
        ExceptionHelper.ThrowIfFalse(idleInterval.TotalSeconds > 0, $"{nameof(idleInterval)}最小单位为秒（s）");
    ...
    /// <returns>空闲对象，自动执行<see cref="IPoolable.Using"/>，标记为using状态；使用完成后执行<see cref="IPoolable.Used"/>方法</returns>
    ...
    /// <param name="addFunc">创建新对象的委托方法</param>
    /// <param name="autoUsing">是否自动执行<see cref="IPoolable.Using"/>方法，将对象标记为使用状态；推荐true，外部手动using，需要做好并发管理</param>
    /// <returns>空闲对象；使用完成后执行<see cref="IPoolable.Used"/>方法</returns>
