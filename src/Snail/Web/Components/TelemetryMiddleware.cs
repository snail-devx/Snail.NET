...
namespace Snail.Web.Components
{
    /// <summary>
    /// HTTP遥测中间件
    /// <para>1、拦截请求和响应结果，记录相关日志</para>
    /// <para>2、发送请求时，附带标准遥测相关数据</para>
    /// </summary>
    [Component<IHttpMiddleware>(Key = MIDDLEWARE_Telemetry)]
    public class TelemetryMiddleware : IHttpMiddleware
    {
        #region 属性变量
        /// <summary>
        /// 日志记录器
        ...
        async Task<HttpResponseMessage> IHttpMiddleware.Send(HttpRequestMessage request, IServerOptions server, HttpDelegate next)
        {
            ...
            //  初始化遥测追踪数据，记录日志
            {
                ...
                Logger.Log(new SendLogDescriptor(true)
                {
                    ...
                    MethodName = nameof(IHttpMiddleware.Send),
                    Exception = null,
